    ...
    <x-pagestructure>
        <div class="flex flex-1 flex-col gap-4 p-4">
            ...
            <div class="flex justify-end">
                <form method='POST' action="/delete-invoice">
                    @csrf
                    @method('DELETE')
            ...
            <form action="/update-invoice" method="POST" class="bg-white p-6 rounded-lg shadow-md">
                @csrf
                @method('PUT')
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input type="text" id="customer_name" name="customer_name"
                    value="{{ optional($customers->where('id', $invoice->customer_id)->first())->customer_name }}"
            ...
            <div>
                ...
                <input type="tel" id="customer_phone" name="customer_phone"
                    value="{{ optional($customers->where('id', $invoice->customer_id)->first())->contact_number }}"
            ...
            <div id="invoice-repeater" class="space-y-2">
              @foreach ($itemsDetails as $index => $detail)
                  <div class="grid grid-cols-13 gap-2 mt-2">
                      ...
                      <div class="col-span-2">
                          <select name="product[]" class="p-2 border rounded-md w-full select_item">
                              ...
                              @foreach ($items as $item)
                                  <option value="{{ $item->id }}" {{ $item->id == $detail['product_id'] ? 'selected' : '' }} data-stock="{{ $item->current_stock }}">
                                      {{ $item->product_name }}
                                  </option>
                              @endforeach
                          </select>
                      ...
                      <div class="col-span-1">
                        <input type="number" name="cgst[]" value="{{ $detail['cgst'] }}"
                              class="p-2 border rounded-md cgst w-full" readonly />
                    </div>

                    <div class="col-span-1">
                      <input type="number" name="sgst[]" value="{{ $detail['sgst'] }}"
                            class="p-2 border rounded-md sgst w-full" readonly />
                  </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mt-5">
            ...
            <div>
              ...
              <input type="number" name="discount_percent" id="discount_percent" step="1"  max="100" value="{{$invoice->discount_percent}}"
