<x-layout> 
    <x-pagestructure>
      ...
      <form action="/add-business-invoice" method="POST" class="bg-white p-6 rounded-lg shadow-md">
        ...
        @csrf
        <h2 class="text-xl font-semibold text-gray-700 mb-4">B2B Invoice</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <!-- Customer Name -->
          <div>
            <label for="customer_id" class="block text-sm font-medium text-gray-600">Select <span class="font-bold underline">C</span>ustomer</label>
            <select id="customer_id" name="customer_id" class="w-full p-2 border border-gray-300 bg-white rounded-md" required>
            ...
            <button type="button" onclick="customerModal()" class="font-medium text-sm text-blue-400">
              + Add New Customer
            </button>
        </div>
            ...
            <div>
                ...
                <option value="Card" {{ old('payment_method') == 'Cheque' ? 'selected' : '' }}>Cheque</option>
                <option value="Card" {{ old('payment_method') == 'NEFT' ? 'selected' : '' }}>NEFT</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input name="customer_gst" placeholder="Customer GST" id="gst_number" class="w-full p-2 border border-gray-300 bg-white rounded-md" readonly>
            </div>
...
$(document).ready(function () {
        $('#addCustomerPop').on('submit', function (e) {
            e.preventDefault(); // Prevent regular form submission

            let formData = $(this).serialize();

            $.ajax({
                url: '/add-customer-pop',
                method: 'POST',
                data: formData,
                headers: {
                ...
                success: function (response) {
                    ...
                    $('#addCustomerPop')[0].reset(); // Clear form
                    closeCustomerModal(); // Close modal (your function)
                    
                    // Optionally reload a customer dropdown
                    $('#customer_id').append(`<option value="${response.id}">${response.customer_name}</option>`);
                },
                error: function (xhr) {
                    let errors = xhr.responseJSON?.errors;
                    if (errors) {
                        let messages = Object.values(errors).map(e => e.join(', ')).join('\n');
                        alert('Error:\n' + messages);
                    } else {
...
`);
    ...
    let clientGst = $('#gst_number').val().substring(0, 2);
    let adminGst = $('#admin_gst').text().substring(0, 2);
