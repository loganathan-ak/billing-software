<x-layout>
    <x-pagestructure>
        <div class="flex justify-end m-5">
            <a href="/b2b-invoice"
               class="inline-block bg-[#062242] text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-[#0b355f] transition duration-200">
                ← Back
            </a>
        </div>

        <div id="invoice" class="max-w-5xl mt-10 mx-auto border border-gray-300 rounded-2xl p-10 shadow-2xl bg-white font-sans text-gray-800">
            <!-- Header -->
            ...
            <div class="grid grid-cols-2 gap-8 mb-8 text-sm">
                <div>
                    ...
                    <p class="text-gray-700">{{ $customers->where('id', $invoice->customer_id)->first()->customer_name ?? 'Unknown' }}</p>
                    <p class="text-gray-700">{{ $customers->where('id', $invoice->customer_id)->first()->contact_number ?? 'Unknown' }}</p>
                    <p class="text-gray-700">GST : <span id="gst_number">{{ $customers->where('id', $invoice->customer_id)->first()->gst_number ?? 'Unknown' }}</span></p>   
                </div>
                <div class="text-right">
            ...
            <div class="grid grid-cols-2 gap-8 mt-10 text-sm">
                ...
                <div class="border rounded-lg p-4 shadow bg-gray-50">
                    ...
                    <div class="flex justify-between py-1 igst_td">
                        <span>IGST</span>
                        <span>₹<span class="total_igst">0</span></span>
                    </div>
    ...
    <script>
        ...
        $(document).ready(function($) {
                    ...
                    var getGst = $('#gst_number').text();;
                    let result = getGst.substring(0, 2);
                    ...
                    $('.sgst_td').removeClass('hidden');
                    $('.cgst_td').removeClass('hidden');
                } else {
                    ...
                    $('.sgst_td').addClass('hidden');
                    $('.cgst_td').addClass('hidden');
                }
